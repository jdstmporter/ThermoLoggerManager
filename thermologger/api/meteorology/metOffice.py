...
class MetOfficeProvider(BaseMeteorologyProvider):
    ...
    def url(self):
        return 'https://data.hub.api.metoffice.gov.uk/sitespecific/v0/point/hourly'
    ...
    def headers(self):
        return dict(apikey=self.SPOT_API)


def runMet():
    try:
        r=MetOfficeProvider(GeographicLocation(51.90006, -2.07972))
        json=r()
        print(str(json))
        data = MetOfficeData(json)
        for item in data:
            print(str(item))
    except Exception as e:
        print(str(e))